...
int32_t BSP_TS_GetCapabilities(TS_Capabilities_t *Capabilities) {
  ...
  Ts_Drv->GetCapabilities(Ts_CompObj, Capabilities);
...
int32_t BSP_TS_GetState(TS_State_t *TS_State) {
  ...
  if (Ts_Drv->GetState(Ts_CompObj, &state) < 0) {
    ...
    /* Check accuracy */
    x_diff = (TS_State->TouchX > Ts_Ctx.PreviousX[0])
                 ? (TS_State->TouchX - Ts_Ctx.PreviousX[0])
                 : (Ts_Ctx.PreviousX[0] - TS_State->TouchX);

    y_diff = (TS_State->TouchY > Ts_Ctx.PreviousY[0])
...
int32_t BSP_TS_GetGestureId(uint32_t *GestureId) {
  ...
  if (Ts_Drv->GetGesture(Ts_CompObj, &tmp) < 0) {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  } else {
...
static int32_t FT5336_Probe() {
  ...
  if (FT5336_RegisterBusIO(&FT5336Obj, &IOCtx) != FT5336_OK) {
    ...
    Ts_CompObj = &FT5336Obj;
    Ts_Drv = (TS_Drv_t *)&FT5336_TS_Driver;

    if (Ts_Drv->Init(Ts_CompObj) != FT5336_OK) {
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
  }
